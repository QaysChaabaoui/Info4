...
// On ouvre l'accès au Restaurateur, à l'Admin ET au Livreur
if (
...
foreach ($commandes as &$cmd) {
    if ($cmd['id'] === $id_commande) {
        ...
        // Si un statut est envoyé, on le force en MAJUSCULES (PRÊTE, EN LIVRAISON, LIVRÉE)
        if ($nouveau_statut !== null) {
            $cmd['statut'] = strtoupper(trim($nouveau_statut));
        }

        // Si la cuisine assigne un livreur à la commande
